...
for i in range(0, 22):
    ...
    tmp.to_csv('output33_' + fname, header=True)

from sklearn import neighbors
model = neighbors.KNeighborsRegressor(7, weights='uniform')   # 这里3是参数，可以换成5，7，9，11，...
...
for j in range(0, 22):
            fname = "output33_T-" + str(j + 1) + ".csv"
            tmp = pd.read_csv(fname)
            tmp['KNN'] = pd.DataFrame(pred[j * lines_per_file: (j + 1) * lines_per_file])
