...
for i in range(0, 22):
    ...
    tmp.to_csv('output2_' + fname, header=True)

# 导入深度学习库 - keras相关部分
from sklearn.metrics import mean_squared_error
from sklearn.svm import LinearSVR
model = LinearSVR(random_state=0, max_iter=2000, C=1)
...
for j in range(0, 22):
            fname = "output2_T-" + str(j + 1) + ".csv"
            tmp = pd.read_csv(fname)
            tmp['SVM'] = pd.DataFrame(pred[j * lines_per_file: (j + 1) * lines_per_file])
